...
C
C      WRITE (LOUT,15)
C   15 FORMAT (/1X,' CKLIB:  Chemical Kinetics Library',
C     1        /1X,'         CHEMKIN-II Version 4.9, April 1994',
C*****precision > double
C     2        /1X,'         DOUBLE PRECISION')
C*****END precision > double
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKKFKRSR (T, C, ICKWRK, RCKWRK, FWDK, REVK)
C
C  START PROLOGUE
C
C  SUBROUTINE CKKFKRSR (T, C, ICKWRK, RCKWRK, FWDK, REVK)
C     Returns the forward and reverse reaction rates for the
C     reactions given temperature and mole concentration.
C
C  Author    - Shirong Bai
C  Date      - 10/24/2014
C  SR        - Stands for Shirong
C
C  INPUT
C     T      - Temperature.
C                   cgs units - K
...
C*****END precision > single
C
      DIMENSION ICKWRK(*), RCKWRK(*), C(*), FWDK(*), REVK(*)
C
      INCLUDE 'ckstrt.f'
C
      CALL CKRATT (RCKWRK, ICKWRK, NII, MXSP, RCKWRK(NcRU),
