...
# parse reactions
def parse_rxn_and_its_index_v2(filename):
    """
    parse reaction info from file= "os.path.join(file_dir, "output", "reaction_labelling.csv")"
    """
    # load data
    line_content = np.genfromtxt(filename, dtype=str, delimiter='\n')
    # use regex to parse reaction itself
    matched_rxn_str = [re.findall("(\d+)\s([-]?\d+)\s+([\w|+|=|>|<|(|)|\-|,]+)", line)[0] for line in line_content \
                       if len(re.findall("(\d+)\s([-]?\d+)\s+([\w|+|=|>|<|(|)|\-|,]+)", line)) != 0]

    # split reactant and product
    reactant_product = [re.findall("([\w|+|(|)|\-|,]+)[=|>|<]+([\w|+|(|)|\-|,]+)", ind_rxn[2]) \
                        for ind_rxn in matched_rxn_str]
    # map reaction new reaction label and the exact reaction
    new_ind_rxn_dict = dict()
    for i in xrange(len(matched_rxn_str)):
        if int(matched_rxn_str[i][1]) >= 0:
            new_ind_rxn_dict.update(
                {matched_rxn_str[i][0]: reactant_product[i][0][0] + '=>' + reactant_product[i][0][1]})
        elif int(matched_rxn_str[i][1]) < 0:
            new_ind_rxn_dict.update(
                {matched_rxn_str[i][0]: reactant_product[i][0][1] + '=>' + reactant_product[i][0][0]})
    return new_ind_rxn_dict
...
def read_pathname_convert_2_real_spe_rxn_v2(filename_spe, filename_rxn, filename_p, filename_p_out, topN=50):
    """
    read species and reaction info, convert path info into real species and reaction instead of index and write to file
    """
    # load path data
    ...
    # load spe and reaction info
    spe_ind_name_dict, spe_name_ind_dict = parse_spe_info(filename_spe)
    new_ind_rxn_dict = parse_rxn_and_its_index_v2(filename_rxn)
