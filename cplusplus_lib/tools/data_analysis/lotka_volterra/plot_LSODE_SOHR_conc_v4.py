...
if __name__ == '__main__':
    ...
    time_SOHR = np.loadtxt(os.path.join(file_dir, "output", "time_SOHR_M_1.csv"), delimiter=",")
    print np.shape(time_SOHR)
    # concentration from SOHR solution
    concentration_SOHR = np.loadtxt(os.path.join(file_dir, "output", "concentration_SOHR_M_1.csv"), delimiter=",",
                                     dtype=float)
