...
def update_settings(file_dir, t_iter_N=None, dt=None):
    ...
    if tin == 0:
        ...
        concentration = rll.read_concentration(os.path.join(file_dir, 'output', 'concentration_SOHR_M_all.csv'))
        curr_settings['chem_init']['species_index_concentration'] = concentration

        temperature = rll.read_temperature(os.path.join(file_dir, 'output', 'temperature_SOHR_M_all.csv'))
        curr_settings['chem_init']['init_temperature'] = temperature

        pressure = rll.read_pressure(os.path.join(file_dir, 'output', 'pressure_SOHR_M_all.csv'))
