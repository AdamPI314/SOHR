...
if __name__ == '__main__':
    ...
    int_drc_dlsode = np.loadtxt(os.path.join(
        file_dir, "output", "int_drc_dlsode_fraction.csv"), delimiter=",", dtype=float)
    int_drc_srode = np.loadtxt(
        os.path.join(file_dir, "output", "int_drc_srode_fraction_" + str(iteration) + ".csv"),
        delimiter=",",
        dtype=float)
    labels = ['$O_2$', '$H_2O$', '$H_2$',
              '$H_2O_2$', '$H$', '$OH$', '$HO_2$', '$O$']
    ...
    ax.plot(time_dlsode, int_drc_dlsode[:, args.s], '--', color=colors[args.s % len(colors)], linewidth=2.5,
            label='dlsode ' + labels[args.s])
    ax.plot(time_srode, int_drc_srode[:, args.s], linestyle='-', color=colors[args.s % len(colors)],
            linewidth=2.5,
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(file_dir, "output", "int_drc_dlsode_srode_" + str(iteration) + "_" +
