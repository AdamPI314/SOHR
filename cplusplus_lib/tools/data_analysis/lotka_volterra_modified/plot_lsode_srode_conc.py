...
if __name__ == '__main__':
    ...
    # concentration
    conc = np.loadtxt(os.path.join(file_dir, "output", "concentration.csv"), delimiter=",")
    print np.shape(conc)

    # concentration from srode solution
    conc_srode = np.loadtxt(os.path.join(file_dir, "output", "concentration_srode.csv"), delimiter=",", dtype=float)
    ...
    counter = 0
    ax.plot(time_srode, conc_srode[:, 0], linestyle='--', marker= markers[counter%len(markers)], markersize=2, color=colors[counter%len(colors)], label= 'drc A 0'); counter+=1
    ax.plot(time_srode, conc_srode[:, 1], linestyle='--', marker= markers[counter%len(markers)], markersize=2, color=colors[counter%len(colors)], label= 'drc X 0'); counter+=1
    ax.plot(time_srode, conc_srode[:, 2], linestyle='--', marker= markers[counter%len(markers)], markersize=2, color=colors[counter%len(colors)], label= 'drc Y 0'); counter+=1
    ax.plot(time_srode, conc_srode[:, 3], linestyle='--', marker= markers[counter%len(markers)], markersize=2, color=colors[counter%len(colors)], label= 'drc Z 0'); counter+=1
